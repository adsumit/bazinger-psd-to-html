...
| File | What it does |
|---|---|
| `extract_psd_spec.py` → `psd-spec.json` | Dumps every PSD layer's properties (geometry, type, fill, effects) to JSON — the machine-readable source of truth. |
| `style-audit.mjs` + `checks.json` | Renders the page and compares each element's **computed CSS** to the spec. Exact, no image alignment — the workhorse. |
| `visual-diff.mjs` | Pixel-diffs a screenshot against `bazinger.png` → a red heatmap (`diff.png`). The catch-all net. |
...
node style-audit.mjs                     # computed-style audit -> PASS/FAIL per property
node visual-diff.mjs index.html bazinger.png   # pixel-diff -> diff.png
```
