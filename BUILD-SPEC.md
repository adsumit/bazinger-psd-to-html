...
## 8. Section 2 — Hero / banner (static frame; 3-slide JS slider deferred)

> Built as a **static frame matching the PNG's single slide + its decorative chrome** (dots/arrows are visual only — no JS cycling yet). Geometry verified element-by-element vs psd-spec + a banner heatmap (0–760); several values **eyeball-corrected vs the PNG** (the PNG outranks layer data — like the navbar 12.4/48.5 in §2).

- **Height 760px** — PSD `Header/Base` bbox bottom = 760, Features starts y762 (*not* the old master-space 800). Background on a **`.banner_bg` child div** (absolute, fills the banner so it can be `transform`ed independently of the content), `background:url(hero_bg.png) center top / cover no-repeat`. `hero_bg.png` is the **blurred cityscape ONLY** (1404 wide = canvas; no phone/text baked in). `.banner{position:relative;overflow:hidden}`, `.banner .center{position:relative;z-index:1}`.
- **Foreground iPhone** — `img/iPhone 5S.png` (PSD `Header/Base/iPhone 5S`, 644×724), a transparent-PNG overlay inside `.banner .center` at `left:-198px; top:176px; width:644px; z-index:-1` (above the bg, behind the text). PSD bbox x[-51→593] y[176→900].
- **Heading** — one `<h1>`, two spans: "Simple, Beautiful " Lato-Light **300** + "and Amazing" Lato-Bold **700**, **46px** `#FFFFFF`, `text-shadow:0 1px 3px rgba(25,22,24,.36)` (§6), `line-height:1`, `margin-top:330px` (PSD heading top y330). **NO blue underline accent** — the old spec claimed one; it does **not** exist in psd-spec or the PNG.
- **Paragraph** — Lato-Light **300** **17px** `#FFFFFF`, no shadow, `width:680px` + **`line-height:19px`** so Chromium's slightly-wider Lato-Light still breaks in **3 lines** like the PSD (57px box). PSD bbox x619–1250 y396–453.
- **"Aavailable on :"** Lato-Light **300** 17px — **the typo is in the PSD; keep it.**
- **Store circles** — `.appleicon`/`.androidicon` **53×53**, `background:rgba(0,0,0,.35)` (PSD container at 35% layer opacity; the glyph stays opaque), `border-radius:50%`, 12px apart, at PSD y597. Apple glyph `#4BCAFF`, Android glyph `#FFFFFF` (apple uses `width:23px; height:auto` + a small `padding:0 4px 2px 0` nudge).
- **Buttons** (PSD **188×64** / **188×61**, `padding:21–22px 50px`, 35px apart): DOWNLOAD = transparent + **2px** `#FFFFFF` border (PSD Stroke size 2), Lato-Bold **700** 14px `#FFFFFF`; LEARN MORE = `#4BCAFF`, Lato-Bold **700** 14px `#FEFEFE`, **`box-shadow:0 4px 0 #277697`** (PSD hard DropShadow #277697, distance 4, size 0 — *not* a border-bottom).
- **Dots (static indicators)** — flex row in `.center`, `left:50%; transform:translateX(-54%); bottom:19px; gap:10px`. Active (slide 1) = **14.6px** solid `#4BCAFF` + 1px ring same; inactive ×2 = **11px** transparent + 1px `#FFFFFF`. (PSD `Slider nav` x671–729 y725–742; eyeballed off the raw 17/13/9.)
- **Arrows (prev/next)** — **51px** circles in `.center`, `top:46.4%; transform:translate(-2%)`, anchored to the content `.prev{left:-78.3px}` / `.next{right:-81px}` (centres land at PSD x93 / x1311, y378). Fill `#4BCAFF`: **left/prev = default** `rgba(75,202,255,.2)` (20%) + white chevron; **right/next = the PSD's frozen hover** — solid `#4BCAFF` + dark `#414042` chevron (same trick as feature-box 3). Chevron = inline SVG, `stroke-width:1`. `:hover` → solid `#4BCAFF` + `#414042` chevron, `transition:background/color 300ms ease`.
- **Deferred:** the functional 3-slide vanilla-JS slider (cycling/auto-advance) — slides 2–3 content isn't in the PNG. The chrome above is the static frame.
